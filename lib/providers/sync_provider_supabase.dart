      ...
      switch (instruccionesBitacora[i].instruccion) {
        case "syncAddUserImage":
          ...
          if (user != null) {
            ...
            final instruccionNoSincronizada = InstruccionNoSincronizada(
                instruccion:
                    "Problems sync to Local Server, User not recovered.",
                fecha: instruccionesBitacora[i].fechaRegistro);
            instruccionesFallidas.add(instruccionNoSincronizada);
            continue;
          }
        ...
        case "syncAddControlFormWeekly":
          final controlFormToSync = getFirstControlForm(
              dataBase.controlFormBox.getAll(), instruccionesBitacora[i].id);
          if (controlFormToSync != null) {
            final responseSyncAddControlForm = await syncAddControlFormWeekly(
                controlFormToSync, instruccionesBitacora[i]);
            if (responseSyncAddControlForm.exitoso) {
              banderasExistoSync.add(responseSyncAddControlForm.exitoso);
              continue;
            ...
            final instruccionNoSincronizada = InstruccionNoSincronizada(
                instruccion:
                    "Problems sync to Local Server, Control Form Check Up Weekly not recovered.",
                fecha: instruccionesBitacora[i].fechaRegistro);
            instruccionesFallidas.add(instruccionNoSincronizada);
            continue;
          }
        case "syncAddOilChangeService":
        ...
        case "syncAddFluidTransmissionChangeService":
          final vehicleServices = getFirstVehicleServices(
              dataBase.vehicleServicesBox.getAll(), instruccionesBitacora[i].id);
          if (vehicleServices != null) {
            final responseSyncUpdateVehicleServices = await syncAddVehicleService(
        ...
        case "syncAddFluidRadiatorChangeService":
          final vehicleServices = getFirstVehicleServices(
              dataBase.vehicleServicesBox.getAll(), instruccionesBitacora[i].id);
          if (vehicleServices != null) {
            final responseSyncUpdateVehicleServices = await syncAddVehicleService(
        ...
        case "syncUpdateMileageRemainingVehicleServices":
          final vehicleServices = getFirstVehicleServices(
              dataBase.vehicleServicesBox.getAll(), instruccionesBitacora[i].id);
          if (vehicleServices != null) {
            final responseSyncUpdatePassword = await syncUpdateMileageRemainingVehicleServices(
                vehicleServices, instruccionesBitacora[i]);
      ...
      if (bitacora.executeSupabase == false) {
        if (controlForm.idDBR == null) {
          ...
          if (recordMeasure.isNotEmpty && recordLights.isNotEmpty && recordCarBodywork.isNotEmpty && recordFluidsCheck.isNotEmpty 
            ...
            if (recordControlForm.isNotEmpty && recordUpdateVehicle.isNotEmpty) {
  ...
  Future<SyncInstruction> syncUpdateVehicleServices(
      ...
      if (bitacora.executeSupabase == false) {
        ...
        if (vehicleServices.service.target!.service == "Oil Change") {
          final recordVehicle = await supabaseCtrlV.from('vehicle').update(
            {
              'oil_change_due': vehicleServices.serviceDate == null ? null : DateFormat('MM/dd/yyyy').format(vehicleServices.serviceDate!),
              'rule_oil_change': {
                ...
                'Last Mileage Service': vehicleServices.vehicle.target!.mileage.toString(),
              },
            },
          ...
          if (recordVehicle.isEmpty) {
            ...
            exitoso: false,
            descripcion:
                "Failed to sync update Rule and Date Vehicle Service Oil Change on Local Server: Vehicle Service 'Oil Change' on Vehicle with License Plates '${vehicleServices.vehicle.target!.licensePlates}'.");
          }
        }
        if (vehicleServices.service.target!.service == "Transmission Fluid Change") {
          final recordVehicle = await supabaseCtrlV.from('vehicle').update(
            {
              'last_transmission_fluid_change': vehicleServices.serviceDate == null ? null : DateFormat('MM/dd/yyyy').format(vehicleServices.serviceDate!),
          ...
          if (recordVehicle.isEmpty) {
            ...
            exitoso: false,
            descripcion:
                "Failed to sync update Rule and Date Vehicle Service Transmission Fluid Change on Local Server: Vehicle Service 'Transmission Fluid Change' on Vehicle with License Plates '${vehicleServices.vehicle.target!.licensePlates}'.");
          }
        }
        if (vehicleServices.service.target!.service == "Radiator Fluid Change") {
          final recordVehicle = await supabaseCtrlV.from('vehicle').update(
            {
              'last_radiator_fluid_change': vehicleServices.serviceDate == null ? null : DateFormat('MM/dd/yyyy').format(vehicleServices.serviceDate!),
          ...
          if (recordVehicle.isEmpty) {
            ...
            exitoso: false,
            descripcion:
                "Failed to sync update Rule and Date Vehicle Service Radiator Fluid Change on Local Server: Vehicle Service 'Radiator Fluid Change' on Vehicle with License Plates '${vehicleServices.vehicle.target!.licensePlates}'.");
          }
        }
    ...
    try {
        ...
        if (controlForm.idDBR == null) {
              ...
              //Parsear a Uint8List
                ...
                if (storageResponse.isEmpty) {
                  return SyncInstruction(
                    exitoso: false,
                    descripcion:
                        "Failed to sync image on Gas-Measures in Control Form Check Up Weekly on Local Server named ${controlForm.measuresR.target!.gasNames.toList()[i]}");
                }
                final urlImage = supabase.storage.from('measures').getPublicUrl(controlForm.measuresR.target!.gasNames.toList()[i]);
                  gasImages = "$gasImages$urlImage|";
              } else {
              ...
              if (!listNamesMeasures.contains(controlForm.measuresR.target!.mileageNames.toList()[i])) {
                ...
                if (storageResponse.isEmpty) {
                  return SyncInstruction(
                    exitoso: false,
                    descripcion:
                        "Failed to sync image on Mileage-Measures in Control Form Check Up Weekly on Local Server named ${controlForm.measuresR.target!.mileageNames.toList()[i]}");
                }
                final urlImage = supabase.storage.from('measures').getPublicUrl(controlForm.measuresR.target!.mileageNames.toList()[i]);
                  mileageImages = "$mileageImages$urlImage|";
              } else {
              ...
              if (!listNamesLights.contains(controlForm.lightsR.target!.headLightsNames.toList()[i])) {
                ...
                if (storageResponse.isEmpty) {
                  return SyncInstruction(
                    exitoso: false,
                    descripcion:
                        "Failed to sync image on Head-Lights in Control Form Check Up Weekly on Local Server named ${controlForm.lightsR.target!.headLightsNames.toList()[i]}");
                }
                final urlImage = supabase.storage.from('lights').getPublicUrl(controlForm.lightsR.target!.headLightsNames.toList()[i]);
                  headlightsImages = "$headlightsImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.lightsR.target!.brakeLightsImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Brake-Lights in Control Form Check Up Weekly on Local Server named ${controlForm.lightsR.target!.brakeLightsNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('lights').getPublicUrl(controlForm.lightsR.target!.brakeLightsNames.toList()[i]);
                brakeLightsImages = "$brakeLightsImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.lightsR.target!.reverseLightsImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Reverse-Lights in Control Form Check Up Weekly on Local Server named ${controlForm.lightsR.target!.reverseLightsNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('lights').getPublicUrl(controlForm.lightsR.target!.reverseLightsNames.toList()[i]);
                reverseLightsImages = "$reverseLightsImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.lightsR.target!.warningLightsImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Warning-Lights in Control Form Check Up Weekly on Local Server named ${controlForm.lightsR.target!.warningLightsNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('lights').getPublicUrl(controlForm.lightsR.target!.warningLightsNames.toList()[i]);
                warningLightsImages = "$warningLightsImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.lightsR.target!.turnSignalsImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on turn Signals in Control Form Check Up Weekly on Local Server named ${controlForm.lightsR.target!.turnSignalsNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('lights').getPublicUrl(controlForm.lightsR.target!.turnSignalsNames.toList()[i]);
                turnSignalsImages = "$turnSignalsImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.lightsR.target!.fourWayFlashersImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Four Way Flashers in Control Form Check Up Weekly on Local Server named ${controlForm.lightsR.target!.fourWayFlashersNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('lights').getPublicUrl(controlForm.lightsR.target!.fourWayFlashersNames.toList()[i]);
                fourWayFlashersImages = "$fourWayFlashersImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.lightsR.target!.dashLightsImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Dash-Lights in Control Form Check Up Weekly on Local Server named ${controlForm.lightsR.target!.dashLightsNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('lights').getPublicUrl(controlForm.lightsR.target!.dashLightsNames.toList()[i]);
                dashLightsImages = "$dashLightsImages$urlImage|";
            }
            ...
            for (var i = 0; i < controlForm.lightsR.target!.strobeLightsImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Strobe-Lights in Control Form Check Up Weekly on Local Server named ${controlForm.lightsR.target!.strobeLightsNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('lights').getPublicUrl(controlForm.lightsR.target!.strobeLightsNames.toList()[i]);
                strobeLightsImages = "$strobeLightsImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.lightsR.target!.cabRoofLightsImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Cab Roof Lights in Control Form Check Up Weekly on Local Server named ${controlForm.lightsR.target!.cabRoofLightsNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('lights').getPublicUrl(controlForm.lightsR.target!.cabRoofLightsNames.toList()[i]);
                cabRoofLightsImages = "$cabRoofLightsImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.lightsR.target!.clearanceLightsImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Clearance-Lights in Control Form Check Up Weekly on Local Server named ${controlForm.lightsR.target!.clearanceLightsNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('lights').getPublicUrl(controlForm.lightsR.target!.clearanceLightsNames.toList()[i]);
                clearanceLightsImages = "$clearanceLightsImages$urlImage|";
              } else {
          ...
          if (controlForm.carBodyworkR.target!.wiperBladesFrontImages.isNotEmpty) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Wiper Blades Front in Control Form Check Up Weekly on Local Server named ${controlForm.carBodyworkR.target!.wiperBladesFrontNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('car-bodywork').getPublicUrl(controlForm.carBodyworkR.target!.wiperBladesFrontNames.toList()[i]);
                wiperBladesFrontImages = "$wiperBladesFrontImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.carBodyworkR.target!.wiperBladesBackImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Wiper Blades Back in Control Form Check Up Weekly on Local Server named ${controlForm.carBodyworkR.target!.wiperBladesBackNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('car-bodywork').getPublicUrl(controlForm.carBodyworkR.target!.wiperBladesBackNames.toList()[i]);
                wiperBladesBackImages = "$wiperBladesBackImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.carBodyworkR.target!.windshieldWiperFrontImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Windshield Wiper Front in Control Form Check Up Weekly on Local Server named ${controlForm.carBodyworkR.target!.windshieldWiperFrontNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('car-bodywork').getPublicUrl(controlForm.carBodyworkR.target!.windshieldWiperFrontNames.toList()[i]);
                windshieldWiperFrontImages = "$windshieldWiperFrontImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.carBodyworkR.target!.windshieldWiperBackImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Windshield Wiper Back in Control Form Check Up Weekly on Local Server named ${controlForm.carBodyworkR.target!.windshieldWiperBackNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('car-bodywork').getPublicUrl(controlForm.carBodyworkR.target!.windshieldWiperBackNames.toList()[i]);
                windshieldWiperBackImages = "$windshieldWiperBackImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.carBodyworkR.target!.generalBodyImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on General Body in Control Form Check Up Weekly on Local Server named ${controlForm.carBodyworkR.target!.generalBodyNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('car-bodywork').getPublicUrl(controlForm.carBodyworkR.target!.generalBodyNames.toList()[i]);
                generalBodyImages = "$generalBodyImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.carBodyworkR.target!.decalingImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Decaling in Control Form Check Up Weekly on Local Server named ${controlForm.carBodyworkR.target!.decalingNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('car-bodywork').getPublicUrl(controlForm.carBodyworkR.target!.decalingNames.toList()[i]);
                decalingImages = "$decalingImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.carBodyworkR.target!.tiresImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Tires in Control Form Check Up Weekly on Local Server named ${controlForm.carBodyworkR.target!.tiresNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('car-bodywork').getPublicUrl(controlForm.carBodyworkR.target!.tiresNames.toList()[i]);
                tiresImages = "$tiresImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.carBodyworkR.target!.glassImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Glass in Control Form Check Up Weekly on Local Server named ${controlForm.carBodyworkR.target!.glassNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('car-bodywork').getPublicUrl(controlForm.carBodyworkR.target!.glassNames.toList()[i]);
                glassImages = "$glassImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.carBodyworkR.target!.mirrorsImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Mirrors in Control Form Check Up Weekly on Local Server named ${controlForm.carBodyworkR.target!.mirrorsNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('car-bodywork').getPublicUrl(controlForm.carBodyworkR.target!.mirrorsNames.toList()[i]);
                mirrorsImages = "$mirrorsImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.carBodyworkR.target!.parkingImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Parking in Control Form Check Up Weekly on Local Server named ${controlForm.carBodyworkR.target!.parkingNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('car-bodywork').getPublicUrl(controlForm.carBodyworkR.target!.parkingNames.toList()[i]);
                parkingImages = "$parkingImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.carBodyworkR.target!.brakesImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Brakes in Control Form Check Up Weekly on Local Server named ${controlForm.carBodyworkR.target!.brakesNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('car-bodywork').getPublicUrl(controlForm.carBodyworkR.target!.brakesNames.toList()[i]);
                brakesImages = "$brakesImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.carBodyworkR.target!.emgBrakesImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on EMG Brakes in Control Form Check Up Weekly on Local Server named ${controlForm.carBodyworkR.target!.emgBrakesNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('car-bodywork').getPublicUrl(controlForm.carBodyworkR.target!.emgBrakesNames.toList()[i]);
                emgBrakesImages = "$emgBrakesImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.carBodyworkR.target!.hornImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Horn in Control Form Check Up Weekly on Local Server named ${controlForm.carBodyworkR.target!.hornNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('car-bodywork').getPublicUrl(controlForm.carBodyworkR.target!.hornNames.toList()[i]);
                hornImages = "$hornImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.fluidsCheckR.target!.engineOilImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Engine Oil in Control Form Check Up Weekly on Local Server named ${controlForm.fluidsCheckR.target!.engineOilNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('fluids-check').getPublicUrl(controlForm.fluidsCheckR.target!.engineOilNames.toList()[i]);
                engineOilImages = "$engineOilImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.fluidsCheckR.target!.transmissionImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Transmission in Control Form Check Up Weekly on Local Server named ${controlForm.fluidsCheckR.target!.transmissionNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('fluids-check').getPublicUrl(controlForm.fluidsCheckR.target!.transmissionNames.toList()[i]);
                transmissionImages = "$transmissionImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.fluidsCheckR.target!.coolantImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Coolant in Control Form Check Up Weekly on Local Server named ${controlForm.fluidsCheckR.target!.coolantNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('fluids-check').getPublicUrl(controlForm.fluidsCheckR.target!.coolantNames.toList()[i]);
                coolantImages = "$coolantImages$urlImage|";
              } else {
              ...
              if (!listNamesFluidsCheck.contains(controlForm.fluidsCheckR.target!.powerSteeringNames.toList()[i])) {
                ...
                if (storageResponse.isEmpty) {
                  return SyncInstruction(
                    exitoso: false,
                    descripcion:
                        "Failed to sync image on Power Steering in Control Form Check Up Weekly on Local Server named ${controlForm.fluidsCheckR.target!.powerSteeringNames.toList()[i]}");
                }
                final urlImage = supabase.storage.from('fluids-check').getPublicUrl(controlForm.fluidsCheckR.target!.powerSteeringNames.toList()[i]);
                  powerSteeringImages = "$powerSteeringImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.fluidsCheckR.target!.dieselExhaustFluidImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Diesel Exhaust Fluid in Control Form Check Up Weekly on Local Server named ${controlForm.fluidsCheckR.target!.dieselExhaustFluidNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('fluids-check').getPublicUrl(controlForm.fluidsCheckR.target!.dieselExhaustFluidNames.toList()[i]);
                dieselExhaustFluidImages = "$dieselExhaustFluidImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.fluidsCheckR.target!.windshieldWasherFluidImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Windshield Washer Fluid in Control Form Check Up Weekly on Local Server named ${controlForm.fluidsCheckR.target!.windshieldWasherFluidNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('fluids-check').getPublicUrl(controlForm.fluidsCheckR.target!.windshieldWasherFluidNames.toList()[i]);
                windshieldWasherFluidImages = "$windshieldWasherFluidImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.bucketInspectionR.target!.insulatedImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Insulated in Control Form Check Up Weekly on Local Server named ${controlForm.bucketInspectionR.target!.insulatedNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('bucket-inspection').getPublicUrl(controlForm.bucketInspectionR.target!.insulatedNames.toList()[i]);
                insulatedImages = "$insulatedImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.bucketInspectionR.target!.holesDrilledImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Holes Drilled in Control Form Check Up Weekly on Local Server named ${controlForm.bucketInspectionR.target!.holesDrilledNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('bucket-inspection').getPublicUrl(controlForm.bucketInspectionR.target!.holesDrilledNames.toList()[i]);
                holesDrilledImages = "$holesDrilledImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.bucketInspectionR.target!.bucketLinerImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Bucket Liner in Control Form Check Up Weekly on Local Server named ${controlForm.bucketInspectionR.target!.bucketLinerNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('bucket-inspection').getPublicUrl(controlForm.bucketInspectionR.target!.bucketLinerNames.toList()[i]);
                bucketLinerImages = "$bucketLinerImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.securityR.target!.rtaMagnetImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on RTA Mganet in Control Form Check Up Weekly on Local Server named ${controlForm.securityR.target!.rtaMagnetNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('security').getPublicUrl(controlForm.securityR.target!.rtaMagnetNames.toList()[i]);
                rtaMagnetImages = "$rtaMagnetImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.securityR.target!.triangleReflectorsImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Triangle Reflectors in Control Form Check Up Weekly on Local Server named ${controlForm.securityR.target!.triangleReflectorsNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('security').getPublicUrl(controlForm.securityR.target!.triangleReflectorsNames.toList()[i]);
                triangleReflectorsImages = "$triangleReflectorsImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.securityR.target!.wheelChocksImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Wheel Chocks in Control Form Check Up Weekly on Local Server named ${controlForm.securityR.target!.wheelChocksNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('security').getPublicUrl(controlForm.securityR.target!.wheelChocksNames.toList()[i]);
                wheelChocksImages = "$wheelChocksImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.securityR.target!.fireExtinguisherImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Fire Extinguisher in Control Form Check Up Weekly on Local Server named ${controlForm.securityR.target!.fireExtinguisherNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('security').getPublicUrl(controlForm.securityR.target!.fireExtinguisherNames.toList()[i]);
                fireExtinguisherImages = "$fireExtinguisherImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.securityR.target!.firstAidKitSafetyVestImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on First Aid Kit Safety Vest in Control Form Check Up Weekly on Local Server named ${controlForm.securityR.target!.firstAidKitSafetyVestNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('security').getPublicUrl(controlForm.securityR.target!.firstAidKitSafetyVestNames.toList()[i]);
                firstAidKitSafetyVestImages = "$firstAidKitSafetyVestImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.securityR.target!.backUpAlarmImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Backup Alarm in Control Form Check Up Weekly on Local Server named ${controlForm.securityR.target!.backUpAlarmNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('security').getPublicUrl(controlForm.securityR.target!.backUpAlarmNames.toList()[i]);
                backUpAlarmImages = "$backUpAlarmImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.extraR.target!.ladderImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Ladder in Control Form Check Up Weekly on Local Server named ${controlForm.extraR.target!.ladderNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('extras').getPublicUrl(controlForm.extraR.target!.ladderNames.toList()[i]);
                ladderImages = "$ladderImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.extraR.target!.stepLadderImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Step Ladder in Control Form Check Up Weekly on Local Server named ${controlForm.extraR.target!.stepLadderNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('extras').getPublicUrl(controlForm.extraR.target!.stepLadderNames.toList()[i]);
                stepLadderImages = "$stepLadderImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.extraR.target!.ladderStrapsImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Ladder Straps in Control Form Check Up Weekly on Local Server named ${controlForm.extraR.target!.ladderStrapsNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('extras').getPublicUrl(controlForm.extraR.target!.ladderStrapsNames.toList()[i]);
                ladderStrapsImages = "$ladderStrapsImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.extraR.target!.hydraulicFluidForBucketImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Hydraulic Fluid For Bucket in Control Form Check Up Weekly on Local Server named ${controlForm.extraR.target!.hydraulicFluidForBucketNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('extras').getPublicUrl(controlForm.extraR.target!.hydraulicFluidForBucketNames.toList()[i]);
                hydraulicFluidForBucketImages = "$hydraulicFluidForBucketImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.extraR.target!.fiberReelRackImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Fiber Reel Rack in Control Form Check Up Weekly on Local Server named ${controlForm.extraR.target!.fiberReelRackNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('extras').getPublicUrl(controlForm.extraR.target!.fiberReelRackNames.toList()[i]);
                fiberReelRackImages = "$fiberReelRackImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.extraR.target!.binsLockedAndSecureImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Bins Locked And Secure in Control Form Check Up Weekly on Local Server named ${controlForm.extraR.target!.binsLockedAndSecureNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('extras').getPublicUrl(controlForm.extraR.target!.binsLockedAndSecureNames.toList()[i]);
                binsLockedAndSecureImages = "$binsLockedAndSecureImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.extraR.target!.safetyHarnessImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Safety Harness in Control Form Check Up Weekly on Local Server named ${controlForm.extraR.target!.safetyHarnessNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('extras').getPublicUrl(controlForm.extraR.target!.safetyHarnessNames.toList()[i]);
                safetyHarnessImages = "$safetyHarnessImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.extraR.target!.lanyardSafetyHarnessImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Lanyard Safety Harness in Control Form Check Up Weekly on Local Server named ${controlForm.extraR.target!.lanyardSafetyHarnessNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('extras').getPublicUrl(controlForm.extraR.target!.lanyardSafetyHarnessNames.toList()[i]);
                lanyardSafetyHarnessImages = "$lanyardSafetyHarnessImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.equipmentR.target!.ignitionKeyImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Ignition Key in Control Form Check Up Weekly on Local Server named ${controlForm.equipmentR.target!.ignitionKeyNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('equipment').getPublicUrl(controlForm.equipmentR.target!.ignitionKeyNames.toList()[i]);
                ignitionKeyImages = "$ignitionKeyImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.equipmentR.target!.binsBoxKeyImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Bins Box Key in Control Form Check Up Weekly on Local Server named ${controlForm.equipmentR.target!.binsBoxKeyNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('equipment').getPublicUrl(controlForm.equipmentR.target!.binsBoxKeyNames.toList()[i]);
                binsBoxKeyImages = "$binsBoxKeyImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.equipmentR.target!.vehicleInsuranceCopyImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Vehicle Insurance Copy in Control Form Check Up Weekly on Local Server named ${controlForm.equipmentR.target!.vehicleInsuranceCopyNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('equipment').getPublicUrl(controlForm.equipmentR.target!.vehicleInsuranceCopyNames.toList()[i]);
                vehicleInsuranceCopyImages = "$vehicleInsuranceCopyImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.equipmentR.target!.vehicleRegistrationCopyImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Vehicle Registration Copy in Control Form Check Up Weekly on Local Server named ${controlForm.equipmentR.target!.vehicleRegistrationCopyNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('equipment').getPublicUrl(controlForm.equipmentR.target!.vehicleRegistrationCopyNames.toList()[i]);
                vehicleRegistrationCopyImages = "$vehicleRegistrationCopyImages$urlImage|";
              } else {
            ...
            for (var i = 0; i < controlForm.equipmentR.target!.bucketLiftOperatorManualImages.toList().length; i++) {
              ...
              if (storageResponse.isEmpty) {
                return SyncInstruction(
                  exitoso: false,
                  descripcion:
                      "Failed to sync image on Bucket Lift Operator Manual in Control Form Check Up Weekly on Local Server named ${controlForm.equipmentR.target!.bucketLiftOperatorManualNames.toList()[i]}");
              }
              final urlImage = supabase.storage.from('equipment').getPublicUrl(controlForm.equipmentR.target!.bucketLiftOperatorManualNames.toList()[i]);
                bucketLiftOperatorManualImages = "$bucketLiftOperatorManualImages$urlImage|";
              } else {
          ...
          if (recordMeasure.isNotEmpty && recordLights.isNotEmpty && recordCarBodywork.isNotEmpty && recordFluidsCheck.isNotEmpty 
            ...
            if (recordControlForm.isNotEmpty && recordUpdateVehicle.isNotEmpty) {
              ...
              exitoso: false,
              descripcion:
                  "Failed to sync all data Control Form Check Up Weekly on Local Server: Control Form with vehicle ID ${controlForm.vehicle.target!.idDBR}.");
            }
          } else {
            return SyncInstruction(
              exitoso: false,
              descripcion:
                  "Failed to sync data Control Form Check Up Weekly on Local Server: Control Form with vehicle ID ${controlForm.vehicle.target!.idDBR}.");
          }
      ...
      return SyncInstruction(
          exitoso: false,
          descripcion:
              "Failed to sync data Control Form Check Up Weekly on Local Server with vehicle ID ${controlForm.vehicle.target!.idDBR}:, details: '$e'");
    }
  }
  ...
  Future<SyncInstruction> syncAddVehicleService(
    ...
    try {
      if (bitacora.executeSupabase == false) {
        ...
        final recordVehicleServices = await supabaseCtrlV.from('vehicle_services').insert(
          {
        ...
        if (vehicleServices.service.target!.service == "Oil Change") {
          final recordVehicle = await supabaseCtrlV.from('vehicle').update(
            {
              'rule_oil_change': {
                'Value': vehicleServices.vehicle.target!.ruleOilChange.target!.value,
                'Registered': vehicleServices.vehicle.target!.ruleOilChange.target!.registered,
                'Last Mileage Service': vehicleServices.vehicle.target!.ruleOilChange.target!.lastMileageService.toString()
                }
            },
          ).eq('id_vehicle', int.parse(vehicleServices.vehicle.target!.idDBR!))
          .select<PostgrestList>("id_vehicle");    
          if (recordVehicle.isEmpty) {
            ...
            exitoso: false,
            descripcion:
                "Failed to sync update Rule Vehicle Service Oil Change on Local Server: Vehicle Service 'Oil Change' on Vehicle with License Plates '${vehicleServices.vehicle.target!.licensePlates}'.");
          }
        }
        if (vehicleServices.service.target!.service == "Transmission Fluid Change") {
          final recordVehicle = await supabaseCtrlV.from('vehicle').update(
            {
              'rule_transmission_fluid_change': {
                'Value': vehicleServices.vehicle.target!.ruleTransmissionFluidChange.target!.value,
                'Registered': vehicleServices.vehicle.target!.ruleTransmissionFluidChange.target!.registered,
                'Last Mileage Service': vehicleServices.vehicle.target!.ruleTransmissionFluidChange.target!.lastMileageService.toString()
              }
            },
          ).eq('id_vehicle', int.parse(vehicleServices.vehicle.target!.idDBR!))
          .select<PostgrestList>("id_vehicle");    
          if (recordVehicle.isEmpty) {
            ...
            exitoso: false,
            descripcion:
                "Failed to sync update Rule Vehicle Service Transmission Fluid Change on Local Server: Vehicle Service 'Transmission Fluid Change' on Vehicle with License Plates '${vehicleServices.vehicle.target!.licensePlates}'.");
          }
        }
        if (vehicleServices.service.target!.service == "Radiator Fluid Change") {
          final recordVehicle = await supabaseCtrlV.from('vehicle').update(
            {
              ...
              'Value': vehicleServices.vehicle.target!.ruleRadiatorFluidChange.target!.value,
              'Registered': vehicleServices.vehicle.target!.ruleRadiatorFluidChange.target!.registered,
              'Last Mileage Service': vehicleServices.vehicle.target!.ruleRadiatorFluidChange.target!.lastMileageService.toString()
              }
            },
          ).eq('id_vehicle', int.parse(vehicleServices.vehicle.target!.idDBR!))
          .select<PostgrestList>("id_vehicle");    
        ...
        if (recordVehicleServices.isNotEmpty) {
          ...
          vehicleServices.idDBR = recordVehicleServices.first['id_vehicle_services'].toString();
          ...
          exitoso: false,
          descripcion:
              "Failed to sync data Add Vehicle Service Status on Local Server: Vehicle Service '${vehicleServices.service.target!.service}' on Vehicle with License Plates '${vehicleServices.vehicle.target!.licensePlates}'.");
        }
      } else {
        dataBase.bitacoraBox.remove(bitacora.id);
        return SyncInstruction(exitoso: true, descripcion: "");
    ...
    try {
      if (bitacora.executeSupabase == false) {
        final res = await supabase.rpc('change_user_password', params: {
          'current_plain_password': bitacora.instruccionAdicional,
          'new_plain_password': usuario.password,
        });
        if (res != null) {
          //Se marca como ejecutada la instrucción en Bitacora
          bitacora.executeSupabase = true;
