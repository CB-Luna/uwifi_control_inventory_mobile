...
//Función inicial para recuperar el Token para la sincronización/posteo de datos
      ...
      try {
        ...
        switch (response.statusCode) {
            case 200:
              ...
              String? idProyectoEmiWeb = dataBase.emprendimientosBox.get(idEmprendimiento)?.idEmiWeb;
              if (idProyectoEmiWeb != null) {
                ...
                if (responseGetUsuarioEstatusProyecto.statusCode == 200) {
                  ...
                  if (responseProyecto.payload.idPromotor.toString() == usuarioActual.idEmiWeb && responseProyecto.payload.switchMovil == true) {
                    return 1;
                  } else {
  ...
  Future<int> acceptCotizacion(Inversiones inversion, int idInversionesXProdCotizados, int idEmprendimiento) async {
    try {
  ...
  Future<int> cancelCotizacion(Inversiones inversion, int idInversionesXProdCotizados, int idEmprendimiento) async {
    try {
      switch (await getTokenOAuth(idEmprendimiento)) {
        case 0:
          return 0;
  ...
  Future<int> buscarOtraCotizacion(Inversiones inversion, int idInversionesXProdCotizados, int idEmprendimiento) async {
    try {
      switch (await getTokenOAuth(idEmprendimiento)) {
        case 0:
          return 0;
