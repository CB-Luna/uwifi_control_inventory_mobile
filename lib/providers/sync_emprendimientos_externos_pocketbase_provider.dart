...
import 'package:taller_alex_app_asesor/modelsPocketbase/temporals/get_basic_consultoria_pocketbase.dart';
import 'package:taller_alex_app_asesor/modelsPocketbase/temporals/get_basic_emprendimeinto_pocketbase.dart';
import 'package:taller_alex_app_asesor/modelsPocketbase/temporals/get_basic_imagen_pocketbase.dart';
import 'package:taller_alex_app_asesor/modelsPocketbase/temporals/get_basic_inversion_pocketbase.dart';
import 'package:taller_alex_app_asesor/modelsPocketbase/temporals/get_basic_inversion_x_prod_cotizados.dart';
import 'package:taller_alex_app_asesor/modelsPocketbase/temporals/get_basic_jornada_pocketbase.dart';
import 'package:taller_alex_app_asesor/modelsPocketbase/temporals/get_basic_pagos_pocketbase.dart';
import 'package:taller_alex_app_asesor/modelsPocketbase/temporals/get_basic_producto_emp_pocketbase.dart';
import 'package:taller_alex_app_asesor/modelsPocketbase/temporals/get_basic_producto_solicitado_pocketbase.dart';
import 'package:taller_alex_app_asesor/modelsPocketbase/temporals/get_basic_producto_vendido.dart';
import 'package:taller_alex_app_asesor/modelsPocketbase/temporals/get_basic_productos_cotizados_pocketbase.dart';
import 'package:taller_alex_app_asesor/modelsPocketbase/temporals/get_basic_productos_proyecto_pocketbase.dart';
import 'package:taller_alex_app_asesor/modelsPocketbase/temporals/get_basic_venta_pocketbase.dart';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final comunidad = dataBase.comunidadesBox
            .query(Comunidades_.nombre.equals(basicEmprendimiento.items[0]
                .expand.idEmprendedorFk.expand.idComunidadFk.nombreComunidad))
            .build()
            .findFirst();
        final catalogoProyecto = dataBase.catalogoProyectoBox
            .query(CatalogoProyecto_.nombre.equals(basicEmprendimiento
                    .items[0].expand.idNombreProyectoFk?.nombreProyecto ??
                ""))
            .build()
            .findFirst();
        if (faseEmp != null && comunidad != null) {
          ...
          final nuevoEmprendedor = Emprendedores(
            nombre: basicEmprendimiento
                .items[0].expand.idEmprendedorFk.nombreEmprendedor,
            apellidos: basicEmprendimiento
                .items[0].expand.idEmprendedorFk.apellidosEmp,
            nacimiento: DateTime.parse("2000-02-27 13:27:00"),
            curp: basicEmprendimiento.items[0].expand.idEmprendedorFk.curp,
            integrantesFamilia: basicEmprendimiento
                .items[0].expand.idEmprendedorFk.integrantesFamilia
                .toString(),
            telefono:
                basicEmprendimiento.items[0].expand.idEmprendedorFk.telefono,
            comentarios: basicEmprendimiento
                    .items[0].expand.idEmprendedorFk.comentarios ??
                '',
            idDBR: basicEmprendimiento.items[0].expand.idEmprendedorFk.id,
            idEmiWeb:
                basicEmprendimiento.items[0].expand.idEmprendedorFk.idEmiWeb,
          );
          ...
          var urlJornadas = Uri.parse(
              "$baseUrl/api/collections/jornadas/records?filter=(id_emprendimiento_fk='$idEmprendimiento')&expand=id_tarea_fk.id_imagenes_fk");
          final headers = ({
            "Content-Type": "application/json",
          });
          var responseJornada = await get(urlJornadas, headers: headers);
          var basicJornadas =
              getBasicJornadaPocketbaseFromMap(responseJornada.body);
          if (responseJornada.statusCode == 200) {
            for (var elementJornada in basicJornadas.items) {
              ...
              if (elementJornada.numJornada == 2) {
                ...
                for (var i = 0;
                    i <
                        elementJornada.expand.idTareaFk.expand!.idImagenesFk
                            .toList()
                            .length;
                    i++) {
                  // Se agrega nueva imagen
                  final uInt8ListImagen = base64Decode(elementJornada
                      .expand.idTareaFk.expand!.idImagenesFk
                      .toList()[i]
                      .base64);
                  final tempDir = await getTemporaryDirectory();
                  File file = await File(
                          '${tempDir.path}/${elementJornada.expand.idTareaFk.expand!.idImagenesFk.toList()[i].nombre}')
                      .create();
                  file.writeAsBytesSync(uInt8ListImagen);
                  final nuevaImagenTarea = Imagenes(
                    imagenes: file.path,
                    nombre: elementJornada.expand.idTareaFk.expand!.idImagenesFk
                        .toList()[i]
                        .nombre,
                    path: file.path,
                    base64: elementJornada.expand.idTareaFk.expand!.idImagenesFk
                        .toList()[i]
                        .base64,
                    idDBR: elementJornada.expand.idTareaFk.expand!.idImagenesFk
                        .toList()[i]
                        .id,
                    idEmiWeb: elementJornada
                        .expand.idTareaFk.expand!.idImagenesFk
                        .toList()[i]
                        .idEmiWeb,
                    idEmprendimiento: idEmprendimientoObjectBox!,
          ...
          var urlConsultorias = Uri.parse(
              "$baseUrl/api/collections/consultorias/records?filter=(id_emprendimiento_fk='$idEmprendimiento')&expand=id_ambito_fk,id_area_circulo_fk,id_tarea_fk.id_porcentaje_fk");

          var responseConsultoria =
              await get(urlConsultorias, headers: headers);
          var basicConsultorias =
              getBasicConsultoriaPocketbaseFromMap(responseConsultoria.body);
          if (responseConsultoria.statusCode == 200) {
            for (var elementConsultoria in basicConsultorias.items) {
              ...
              for (var i = 0;
                  i < elementConsultoria.expand.idTareaFk.toList().length;
                  i++) {
                final nuevaTarea = Tareas(
                  tarea: elementConsultoria.expand.idTareaFk[i].tarea,
                  descripcion:
                      elementConsultoria.expand.idTareaFk[i].descripcion,
                  comentarios:
                      elementConsultoria.expand.idTareaFk[i].comentarios,
                  fechaRevision:
                      elementConsultoria.expand.idTareaFk[i].fechaRevision!,
                ...
                final porcentajeAvance = dataBase.porcentajeAvanceBox
                    .query(PorcentajeAvance_.idEmiWeb.equals(elementConsultoria
                        .expand.idTareaFk[i].expand!.idPorcentajeFk.idEmiWeb))
                    .build()
                    .findFirst();
                final ambito = dataBase.ambitoConsultoriaBox
                    .query(AmbitoConsultoria_.idEmiWeb
                        .equals(elementConsultoria.expand.idAmbitoFk.idEmiWeb))
                    .build()
                    .findFirst();
                final areaCirculo = dataBase.areaCirculoBox
                    .query(AreaCirculo_.idEmiWeb.equals(
                        elementConsultoria.expand.idAreaCirculoFk.idEmiWeb))
                    .build()
                    .findFirst();
                ...
                if (elementConsultoria
                    .expand.idTareaFk[i].idImagenesFk.isNotEmpty) {
                  final recordGetOneImagenConsultoria = await client.records
                      .getOne(
                          'imagenes',
                          elementConsultoria
                              .expand.idTareaFk[i].idImagenesFk[0]);
                  if (recordGetOneImagenConsultoria.id.isNotEmpty) {
                    final imagenConsultoriaParse =
                        getBasicImagenPocketbaseFromMap(
                            recordGetOneImagenConsultoria.toString());
                    // Se agrega nueva imagen de la tarea
                    final uInt8ListImagen =
                        base64Decode(imagenConsultoriaParse.base64);
                    final tempDir = await getTemporaryDirectory();
                    File file = await File(
                            '${tempDir.path}/${imagenConsultoriaParse.nombre}')
                        .create();
                    file.writeAsBytesSync(uInt8ListImagen);
                    final nuevaImagenTarea = Imagenes(
                      imagenes: file.path,
          ...
          var urlProductosEmp = Uri.parse(
              "$baseUrl/api/collections/productos_emp/records?filter=(id_emprendimiento_fk='$idEmprendimiento')&expand=id_imagen_fk,id_und_medida_fk");

          var responseProductosEmp =
              await get(urlProductosEmp, headers: headers);
          var basicProductosEmp =
              getBasicProductoEmpPocketbaseFromMap(responseProductosEmp.body);
          if (responseProductosEmp.statusCode == 200) {
            for (var elementProductoEmp in basicProductosEmp.items) {
              ...
              final unidadMedida = dataBase.unidadesMedidaBox
                  .query(UnidadMedida_.idEmiWeb
                      .equals(elementProductoEmp.expand.idUndMedidaFk.idEmiWeb))
                  .build()
                  .findFirst();
              ...
              if (elementProductoEmp.expand.idImagenFk?.created != null) {
                ...
                final uInt8ListImagen =
                    base64Decode(elementProductoEmp.expand.idImagenFk!.base64);
                final tempDir = await getTemporaryDirectory();
                File file = await File(
                        '${tempDir.path}/${elementProductoEmp.expand.idImagenFk!.nombre}')
                    .create();
                file.writeAsBytesSync(uInt8ListImagen);
          ...
          var urlVentas = Uri.parse(
              "$baseUrl/api/collections/ventas/records?filter=(id_emprendimiento_fk='$idEmprendimiento')");

          var responseVentas = await get(urlVentas, headers: headers);
          ...
          if (responseVentas.statusCode == 200) {
            for (var elementVenta in basicVentas.items) {
              ...
              var urlProdVendidos = Uri.parse(
                  "$baseUrl/api/collections/prod_vendidos/records?filter=(id_venta_fk='${elementVenta.id}')&expand=id_und_medida_fk");

              var responseProdVendidos =
                  await get(urlProdVendidos, headers: headers);
              var basicProdVendidos = getBasicProductoVendidoPocketbaseFromMap(
                  responseProdVendidos.body);
              if (responseProdVendidos.statusCode == 200) {
                for (var elementProdVendido in basicProdVendidos.items) {
                  ...
                  final productoEmp = dataBase.productosEmpBox
                      .query(ProductosEmp_.idDBR
                          .equals(elementProdVendido.idProductosEmpFk))
                      .build()
                      .findFirst();
                  final unidadMedida = dataBase.unidadesMedidaBox
                      .query(UnidadMedida_.idEmiWeb.equals(
                          elementProdVendido.expand.idUndMedidaFk.idEmiWeb))
                      .build()
                      .findFirst();
          ...
          var urlInversiones = Uri.parse(
              "$baseUrl/api/collections/inversiones/records?filter=(id_emprendimiento_fk='$idEmprendimiento')&expand=id_estado_inversion_fk,id_imagen_firma_recibido_fk,id_imagen_producto_entregado_fk");

          var responseInverisones = await get(urlInversiones, headers: headers);
          var basicInversion =
              getBasicInversionPocketbaseFromMap(responseInverisones.body);
            ...
            for (var elementInversion in basicInversion.items) {
              ...
              final estadoInversion = dataBase.estadoInversionBox
                  .query(EstadoInversion_.idDBR
                      .equals(elementInversion.expand.idEstadoInversionFk.id))
                  .build()
                  .findFirst();
              ...
              if (elementInversion.expand.idImagenFirmaRecibidoFk != null) {
                ...
                final uInt8ListImagen = base64Decode(
                    elementInversion.expand.idImagenFirmaRecibidoFk!.base64);
                final tempDir = await getTemporaryDirectory();
                File file = await File(
                        '${tempDir.path}/${elementInversion.expand.idImagenFirmaRecibidoFk!.nombre}')
                    .create();
                file.writeAsBytesSync(uInt8ListImagen);
                final nuevaImagenFirma = Imagenes(
                  imagenes: file.path,
                  nombre:
                      elementInversion.expand.idImagenFirmaRecibidoFk!.nombre,
                  path: file.path,
                  base64:
                      elementInversion.expand.idImagenFirmaRecibidoFk!.base64,
                  idDBR: elementInversion.expand.idImagenFirmaRecibidoFk!.id,
                  idEmiWeb:
                      elementInversion.expand.idImagenFirmaRecibidoFk!.idEmiWeb,
                  idEmprendimiento: idEmprendimientoObjectBox!,
                );
              ...
              if (elementInversion.expand.idImagenProductoEntregadoFk != null) {
                ...
                final uInt8ListImagen = base64Decode(elementInversion
                    .expand.idImagenProductoEntregadoFk!.base64);
                final tempDir = await getTemporaryDirectory();
                File file = await File(
                        '${tempDir.path}/${elementInversion.expand.idImagenProductoEntregadoFk!.nombre}')
                    .create();
                file.writeAsBytesSync(uInt8ListImagen);
                final nuevaImagenProdEntregado = Imagenes(
                  imagenes: file.path,
                  nombre: elementInversion
                      .expand.idImagenProductoEntregadoFk!.nombre,
                  path: file.path,
                  base64: elementInversion
                      .expand.idImagenProductoEntregadoFk!.base64,
                  idDBR:
                      elementInversion.expand.idImagenProductoEntregadoFk!.id,
                  idEmiWeb: elementInversion
                      .expand.idImagenProductoEntregadoFk!.idEmiWeb,
                  idEmprendimiento: idEmprendimientoObjectBox!,
                );
                nuevaImagenProdEntregado.inversion.target = nuevaInversion;
                nuevaInversion.imagenProductoEntregado.target =
                    nuevaImagenProdEntregado;
              ...
              if (elementInversion.jornada3) {
                ...
                var basicProductosProyecto =
                    getBasicProductosProyectoPocketbaseFromMap(
                        responseProductosProyecto.body);
                if (responseProductosProyecto.statusCode == 200) {
                  for (var elementProductosProyecto
                      in basicProductosProyecto.items) {
                    ...
                    final nuevoProdSolicitado = ProdSolicitado(
                      ...
                      proveedorSugerido:
                          elementProductosProyecto.proveedorSugerido,
                      cantidad: elementProductosProyecto.cantidad,
                      costoEstimado:
                          double.parse(elementProductosProyecto.costoEstimado),
                  ...
                  final nuevaInversionXProdCotizados =
                      InversionesXProdCotizados(
                    idEmprendimiento: idEmprendimientoObjectBox!,
                  ); //Se crea la inversion x prod Cotizados
                  nuevaInversionXProdCotizados.inversion.target =
                      nuevaInversion;
                  dataBase.inversionesXprodCotizadosBox
                      .put(nuevaInversionXProdCotizados);
                  nuevaInversion.inversionXprodCotizados
                ...
                var urlProdSolicitados = Uri.parse(
                    "$baseUrl/api/collections/productos_solicitados/records?filter=(id_inversion_fk='${elementInversion.id}')&expand=id_familia_prod_fk,id_tipo_empaques_fk,id_imagen_fk");

                var responseProdSolicitados =
                    await get(urlProdSolicitados, headers: headers);
                var basicProdSolicitados =
                    getBasicProductoSolicitadoPocketbaseFromMap(
                        responseProdSolicitados.body);
                if (responseProdSolicitados.statusCode == 200) {
                  for (var elementProdSolicitado
                      in basicProdSolicitados.items) {
                    // Se agrega nuevo prod solicitado a la inversion
                    final nuevoProdSolicitado = ProdSolicitado(
                      idInversion: idInversion,
                    ...
                    final tipoEmpaques = dataBase.tipoEmpaquesBox
                        .query(TipoEmpaques_.idDBR
                            .equals(elementProdSolicitado.idTipoEmpaquesFk))
                        .build()
                        .findFirst();
                    if (tipoEmpaques != null) {
                      nuevoProdSolicitado.tipoEmpaques.target = tipoEmpaques;
                    }
                    if (elementProdSolicitado.expand.idImagenFk?.created !=
                        null) {
                      // Se agrega nueva imagen del producto Solicitado
                      final uInt8ListImagen = base64Decode(
                          elementProdSolicitado.expand.idImagenFk!.base64);
                      final tempDir = await getTemporaryDirectory();
                      File file = await File(
                              '${tempDir.path}/${elementProdSolicitado.expand.idImagenFk!.nombre}')
                          .create();
                      file.writeAsBytesSync(uInt8ListImagen);
                      final nuevaImagenProdSolicitado = Imagenes(
                        ...
                        idEmiWeb:
                            elementProdSolicitado.expand.idImagenFk!.idEmiWeb,
                        idEmprendimiento: idEmprendimientoObjectBox!,
                      ); //Se crea el objeto imagenes para el producto Solicitado
                      dataBase.imagenesBox.put(nuevaImagenProdSolicitado);
                      nuevoProdSolicitado.imagen.target =
                          nuevaImagenProdSolicitado;
                    }
                    nuevoProdSolicitado.inversion.target = nuevaInversion;
                    dataBase.productosSolicitadosBox.put(nuevoProdSolicitado);
                    nuevaInversion.prodSolicitados.add(nuevoProdSolicitado);
                ...
                var urlInversionXProdCotizados = Uri.parse(
                    "$baseUrl/api/collections/inversion_x_prod_cotizados/records?filter=(id_inversion_fk='${elementInversion.id}')");

                var responseInversionXProdCotizados =
                    await get(urlInversionXProdCotizados, headers: headers);
                var basicInversionXProdCotizados =
                    getBasicInversionXProdCotizadosPocketbaseFromMap(
                        responseInversionXProdCotizados.body);
                if (responseInversionXProdCotizados.statusCode == 200) {
                  if (basicInversionXProdCotizados.items.toList().isEmpty) {
                    ...
                    for (var elementInversionXProdCotizados
                        in basicInversionXProdCotizados.items) {
                      ...
                      var urlProductosCotizados = Uri.parse(
                          "$baseUrl/api/collections/productos_cotizados/records?filter=(id_inversion_x_prod_cotizados_fk='${elementInversionXProdCotizados.id}')&expand=id_producto_prov_fk");

                      var responseProductosCotizados =
                          await get(urlProductosCotizados, headers: headers);
                      var basicProductosCotizados =
                          getBasicProductosCotizadosPocketbaseFromMap(
                              responseProductosCotizados.body);
                      if (responseProductosCotizados.statusCode == 200) {
                        for (var elementProductosCotizados
                            in basicProductosCotizados.items) {
                          ...
                          final nuevoProductoCotizado = ProdCotizados(
                            ...
                            costoUnitario:
                                (elementProductosCotizados.costoTotal /
                                    elementProductosCotizados.cantidad),
                          ...
                          final productoProv = dataBase.productosProvBox
                              .query(ProductosProv_.idDBR.equals(
                                  elementProductosCotizados.idProductoProvFk))
                              .build()
                              .findFirst();
                          if (productoProv != null) {
                            nuevoProductoCotizado.productosProv.target =
                                productoProv;
                          }
                          nuevoProductoCotizado.inversionXprodCotizados.target =
                              nuevaInversionXProdCotizados;
                          dataBase.productosCotBox.put(nuevoProductoCotizado);
                          nuevaInversionXProdCotizados.prodCotizados
              ...
              var urlPagos = Uri.parse(
                  "$baseUrl/api/collections/pagos/records?filter=(id_inversion_fk='${elementInversion.id}')");

              var responsePagos = await get(urlPagos, headers: headers);
              var basicPagos =
                  getBasicPagosPocketbaseFromMap(responsePagos.body);
      ...
      if (exitoso) {
        ...
        final actualizarUsuarioUri = Uri.parse(
            '$baseUrlEmiWebServices/proyectos/status?idProyecto=$idEmprendimientoEmiWeb');
        final headers = ({
          "Content-Type": "application/json",
