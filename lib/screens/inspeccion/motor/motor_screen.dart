...
import 'package:taller_alex_app_asesor/screens/inspeccion/motor/components/seccion_dos_formulario.dart';
import 'package:taller_alex_app_asesor/screens/inspeccion/motor/components/seccion_uno_formulario.dart';
import 'package:taller_alex_app_asesor/screens/inspeccion/motor/motor_agregado_screen.dart';
...
class MotorScreen extends StatefulWidget {
  ...
  @override
  _MotorScreen createState() => _MotorScreen();
}

class _MotorScreen extends State<MotorScreen> {
                ...
                BottomNavigationBarItem(
                  icon: InkWell(
                    onTap: () async {
                      switch (activeStep) {
                        case 0:
                          if (motorProvider.validarSeccionUnoFormulario()) {
                              setState(() {
                                activeStep++;
                              });
                            } else {
                          ...
                          if (motorProvider.validarSeccionDosFormulario()) {
                              if (motorProvider.agregarMotor(widget.ordenTrabajo)) {
                                ...
                                snackbarKey.currentState?.showSnackBar(const SnackBar(
                                  content: Text("No se pudo agregar información del motor, intente más tarde."),
                                ));
                              }
                            } else {
                              await showDialog(
